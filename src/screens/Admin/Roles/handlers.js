      ...
      },
      {
        ...
        renderCell: (p) => {
            return <div className='d-flex justify-content-evenly w-100'>
                <button className='btn btn-sm btn-success' onClick={()=>{setRol(p.value); setOpenAsignar(true)}}>Asignar</button>
                <button className='btn btn-sm btn-danger' onClick={()=>{setRol(p.value); setOpenDesasignar(true)}}>Desasignar</button>
            </div>
          },
      }
    ];
